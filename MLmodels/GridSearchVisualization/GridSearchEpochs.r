...
pShapeAccuracy <- ggplot(data.long, aes(x=factor(epochs), y=value, fill=factor(epochs), label=valuePercentage)) + 
...
pSizeAccuracy <- ggplot(data.longSize, aes(x=factor(epochs), y=value, fill=factor(epochs), label=valuePercentage)) + 
  geom_bar(stat='identity', position=position_dodge()) +
  geom_text(position = position_dodge(width = 1), vjust = -0.5) +
  #  geom_line() +
  #geom_vline(xintercept = 64, linetype="dotted", color = "red", size=lineSize) +  
...
data.rotation <- melt(data.plot, id = ids, measure = c("test_rotation_output_min_angle_error"))
#data.rotation$dropout = as.factor(data.rotation$dropout)
data.rotation$value = data.rotation$value / pi * 180;
data.rotation$valueLabel = paste(round(data.rotation$value, 2),"°",sep="");

pRotationError <- ggplot(data.rotation, aes(x=factor(epochs), y=value, fill=factor(epochs), label=valueLabel)) + 
  geom_bar(stat='identity', position=position_dodge()) +
  geom_text(position = position_dodge(width = 1), vjust = -0.5) +
  #geom_vline(xintercept = 64, linetype="dotted", color = "red", size=lineSize) +
  #  geom_line() +
...
pShapeLoss <- ggplot(data.loss, aes(x=factor(epochs), y=value, label=value)) + 
  geom_bar(stat='identity', position=position_dodge()) +
  geom_text(position = position_dodge(width = 1), vjust = -0.5) +
...
pSizeLoss <- ggplot(data.sizeloss, aes(x=factor(epochs), y=value,fill=factor(epochs), label=value)) + 
  geom_bar(stat='identity', position=position_dodge()) +
  geom_text(position = position_dodge(width = 1), vjust = -0.5) +
  #geom_text(nudge_y = 1,nudge_x = 2, size=3) +
  #  geom_line() +
...
pRotationLoss <- ggplot(data.loss, aes(x=factor(epochs), y=value, fill=factor(epochs), label=value)) + 
  geom_bar(stat='identity', position=position_dodge()) +
  geom_text(position = position_dodge(width = 1), vjust = -0.5) +
  #geom_text(nudge_y = 1,nudge_x = 2, size=3) +
  # geom_line() +
...
index = 3
cat(paste("You selected the model with this epochs: ",data.raw[index,]$epochs,"\n",sep=""));

cat(paste("\\def\\accuracyBoth{$",round(data.raw[index,]$test_size_output_accuracy*data.raw[index,]$test_shape_output_accuracy*100,2),"\\,\\%$}\n",sep=""));
cat(paste("\\def\\accuracyShape{$",round(data.raw[index,]$test_shape_output_accuracy*100,2),"\\,\\%$}\n",sep=""));
cat(paste("\\def\\accuracySize{$",round(data.raw[index,]$test_size_output_accuracy*100,2),"\\,\\%$}\n",sep=""));
cat(paste("\\def\\rotationError{$",round(data.raw[index,]$test_rotation_output_min_angle_error/pi * 180,2),"\\,^{\\circ}$}\n",sep=""));
cat()
