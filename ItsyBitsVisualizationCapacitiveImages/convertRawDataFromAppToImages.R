...
data <- read.csv("raw/shape_measuring-1970-01-15-19-53-44.csv", sep=",", header=FALSE)
#data <- read.csv("raw/shape_measuring-2020-09-10-06-18-55.csv", sep=",", header=FALSE)
#data <- read.csv("raw/shape_measuring-1970-01-16-02-22-48.csv", sep=",", header=FALSE)
#data <- read.csv("raw/arrow.csv", sep=",", header=FALSE)
...
for (index in c(1:nrow(cap))) {
  ...
  png(filename=paste(folder,"/frame", index, ".png",sep=""), width=x*15, height=x*27, antialias = "none")
  m[m < 0] = 0
  par(mar=c(0, 0, 0, 0))
  #rev desaturate
